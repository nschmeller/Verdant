...
struct ZZThresholds {
    @Test func probe() async throws {
        ...
        try await TestSupport.seed(writer, metric: .restingHeartRate, value: 56, daysAgo: 1...70, jitter: 1, now: now)
        try await TestSupport.seed(writer, metric: .restingHeartRate, value: 63, daysAgo: 71...220, jitter: 1, now: now)
        try await TestSupport.seed(writer, metric: .bodyMass, value: 78, daysAgo: 1...220, jitter: 0.3, now: now)
        ...
        let arms = [
            ("REAL", "Your resting heart rate settled at a lower level — it has averaged about 56 bpm "
                + "since, against about 63 bpm before.\n\nVerified basis: \(real.verifiedBasis)"),
            ("SPUR", "Your body mass settled at a new level — it has averaged about 78 kg since, "
        ...
        for k in 2 ... 6 {
            let real = observed["REAL"]?.filter { $0.held >= k }.count ?? 0
            let spur = observed["SPUR"]?.filter { $0.held >= k }.count ?? 0
        ...
        let realMaj = observed["REAL"]?.filter { $0.held * 2 > $0.rendered }.count ?? 0
        let spurMaj = observed["SPUR"]?.filter { $0.held * 2 > $0.rendered }.count ?? 0
